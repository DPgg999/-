...
class Neo4j_Handle():
	...
	def connectDB(self):
		self.graph = Graph("bolt: // localhost:7474", username="neo4j", password="123456")
		self.matcher = NodeSelection(self.graph)

	#实体查询，用于命名实体识别：品牌+车系+车型
	def matchEntityItem(self,value):
		answer = self.graph.run("MATCH (Car) WHERE Car.title = \""+value+"\" RETURN Car").data()
		# print(answer,"----------------answer")
		return answer
	...
	def getEntityRelationbyEntity(self,value):
		...
		answer = self.graph.run("MATCH (Car) - [relation] - (brand)  WHERE Car.title = \""+value+"\" RETURN relation,brand").data()
		# print("MATCH (Car) - [relation] - (brand)  WHERE Car.title = \""+value+"\" RETURN relation,brand")
		if(len(answer) == 0):
			#查询实体：不考虑关系方向
			answer = self.graph.run("MATCH (Car) - [relation] - (brand)  WHERE Car.title = \""+value+"\" RETURN relation,brand").data()
		print(answer,"---answer")
